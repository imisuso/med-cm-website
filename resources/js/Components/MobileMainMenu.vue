...
<!--    </Link>-->
    <a v-else :href="route(`${menu_link}`)">
        <button class="text-white font-sans font-bold my-3 flex flex-row items-center dropsidemenu">{{ $t(menu_name) }} :</button>
    </a>
    ...
    <ul v-if="has_sub_menu" class="pl-1 border-white text-menu-color dropsidemenu" :class="{'hidden': !items_visibility}">
        <template v-for="sub_menu_item in sub_menu" :key="sub_menu_item.sub_menu_id">
            <li class="flex flex-row items-baseline mb-1 py-1 rounded-sm shadow-md dropsidemenu">
                ...
                <a v-if="sub_menu_item.sub_menu_link.startsWith('#')" :href="sub_menu_item.sub_menu_link" @click="$emit('clickTag')">{{ $t(sub_menu_item.sub_menu_name) }}</a>
                <a v-else-if="sub_menu_item.sub_menu_link.startsWith('http')" :href="sub_menu_item.sub_menu_link" target="_blank" @click="true">{{ $t(sub_menu_item.sub_menu_name) }}</a>
