<template>
  ...
  <div v-else-if="display_content === 'template'" class="flex flex-col">
    ...
    <div class="flex-grow">
      <QuillEditor ref="quill_academic_service"
                  theme="snow"
                  contentType="delta"
                  @ready="initialQuill()"
...
const quill_academic_service = ref()
...
const textChange = (e) => {
  ...
  const deleted = getImgUrls(quill_academic_service.value.getContents().diff(e.oldContents))
...
const initialQuill = () => {
  quill_academic_service.value.getEditor().addEventListener('paste', pasteImage, false)
  quill_academic_service.value.setContents(JSON.parse(props.branchSubMenu.detail_delta))
  quill_academic_service.value.getQuill().enable(false)
}

const editContent = () => {
  quill_academic_service.value.getQuill().enable(true)
...
const insertToEditor = ( url ) => {
  let editor = quill_academic_service.value.getQuill()
...
const saveContent = () => {
  ...
  form.detail_delta = JSON.stringify(quill_academic_service.value.getContents())
  form.detail_html = quill_academic_service.value.getHTML()
