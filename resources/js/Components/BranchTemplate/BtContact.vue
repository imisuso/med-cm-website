...
const textChange = (e) => {
  ...
  const deleted = getImgUrls(quill_contact.value.getContents().diff(e.oldContents))
...
const initialQuill = () => {
  quill_contact.value.getEditor().addEventListener('paste', pasteImage, false)
  quill_contact.value.setContents(JSON.parse(props.branchSubMenu.detail_delta))
  quill_contact.value.getQuill().enable(false)
}
...
const editContent = () => {
  quill_contact.value.getQuill().enable(true)
...
const insertToEditor = ( url ) => {
  let editor = quill_contact.value.getQuill()
...
const saveContent = () => {
  ...
  form.detail_delta = JSON.stringify(quill_contact.value.getContents())
  form.detail_html = quill_contact.value.getHTML()
