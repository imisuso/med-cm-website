<template>
  <div class="px-4 py-8 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8">
    <div class="flex flex-col items-start">   
      ...
      <div v-for="item in announcements.data" :key="item.id" class="flex flex-wrap md:flex-nowrap w-full">
        <div v-if="item.publish_status" class="w-full mb-2 bg-white rounded-md border border-gray-400 border-l-4 border-l-teal-600">
          ...
          <div class="flex flex-col items-start shadow-md rounded-md">
            <svg v-if="item.pinned" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" class="h-5 w-5 text-red-500 shrink-0 ml-2" width="256" height="256" viewBox="0 0 256 256" xml:space="preserve">
...
import { SearchIcon, NewspaperIcon } from '@heroicons/vue/outline';
...
watch( search, value => {
    Inertia.get(route('announce_all_publish'), { search: value }, {
        preserveState: true,
        replace: true
    })
})
