...
<AdminAppLayout>
  <div class="flex flex-col p-4 w-full">
    ...
    <div v-for="item in props.divisions.data" :key="item.id" class="flex flex-col items-center space-y-4 sm:flex-row w-full px-4 py-2 mb-2 mx-auto bg-gradient-to-l from-sky-100 rounded-lg shadow-md dark:bg-gray-800">  
      <div>
        <!-- <svg v-if="item.image === ''" class="h-20 w-20 rounded-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
          <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg> -->
        <!-- <svg v-else class="h-20 w-20 rounded-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
          <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg> 
        <img v-else :src="`${item.image_url}`" class="h-20 w-20 rounded-full object-cover mr-4"/> -->
